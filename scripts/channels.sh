...
#
# `start-server` first (upstream boot-race fix): at boot systemd starts channels
# and dashboard in the same second; without an explicit server, `set-environment -g`
# silently no-ops and our new-session inherits an empty global env -> the channels
# claude comes up "Not logged in". start-server is idempotent and cheap, and makes
# the `set-environment -g` below always land.
