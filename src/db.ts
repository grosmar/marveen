...
export function saveAgentMemory(
  ...
  const now = Math.floor(Date.now() / 1000)
  // Defensive: keywords is a TEXT column. Bind only a string or null — a non-string
  // (e.g. an array from an untyped HTTP caller) would throw an uncaught SQLite bind
  // error and surface as an opaque 500. Callers should normalise upstream; this is the
  // last-line guard so a bad type can never crash a memory write. (2026-07-26)
  const keywordsText = typeof keywords === 'string' ? keywords : null
  ...
  ).run(ALLOWED_CHAT_ID, null, content, 'semantic', now, now, agentId, category, autoGenerated ? 1 : 0, keywordsText)
  ...
  generateEmbedding(content + (keywordsText ? ' ' + keywordsText : '')).then(emb => {
    if (emb) {
